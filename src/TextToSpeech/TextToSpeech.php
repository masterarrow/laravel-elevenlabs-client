...
use Exception;
use ElevenLabsWrapper\ElevenLabsClient\ElevenLabsClient;
...
use ElevenLabsWrapper\ElevenLabsClient\Enums\VoiceSettingsEnum;
use ElevenLabsWrapper\ElevenLabsClient\Responses\ErrorResponse;
use ElevenLabsWrapper\ElevenLabsClient\Responses\SuccessResponse;
use ElevenLabsWrapper\ElevenLabsClient\Traits\ExceptionHandlerTrait;
...
class TextToSpeech implements TextToSpeechInterface
{
   ...
   /**
    ...
    ?array $voice_settings = []) : \GuzzleHttp\Psr7\Response // array
    {
        try {
            ...
            if ($status === 200) {
                return $response; // (new SuccessResponse($status, "Voice Succesfully Generated"))->getResponse();
            }
        } catch (Exception $e) {
